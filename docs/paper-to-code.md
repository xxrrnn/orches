...
| Sec. 2.2, Fig. 3 | Generation/verification TTC tree | TTC v1/v2; generation-only policy schema/manifest; `collectors/policy_events.py`, `compute_optimal_tts.py`, `policy_collection.py`, `host_probe.py`; compute-optimal-TTS token/control-flow patches; `replay_v2.py` | Exact-token round trip/replay; session and event-set success/OOM tests; host/BF16/cell checks; byte-stable trace/manifest/report; combined patch lifecycle | M4B.5A generation collection path complete for `num_sequence=1`; compatible UV lock, GPU run, and PRM evidence pending |
