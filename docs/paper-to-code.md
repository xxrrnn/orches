...
| Sec. 2.2, Fig. 3 | Generation/verification TTC tree | TTC v1/v2; generation-only policy schema/manifest; `collectors/policy_events.py`, `compute_optimal_tts.py`; compute-optimal-TTS token/control-flow patches; `replay_v2.py` | Exact-token round trip/replay; session success/OOM/config/beam tests; vLLM append-only/readiness/materialization; combined patch apply/compile/reverse | M4B.4B generation collector complete for `num_sequence=1`; GPU run and PRM evidence pending |
