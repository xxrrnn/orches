...
| Sec. 2.2, Fig. 3 | Generation/verification TTC tree | TTC v1/v2; generation-only policy schema/manifest; `collectors/policy_events.py`, `compute_optimal_tts.py`; `replay_v2.py` | Exact-token round trip/replay; raw event conversion; vLLM append-only/readiness/materialization; external patch check | M4B.4A worker evidence complete; selection hook and real run pending |
